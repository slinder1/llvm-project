...
class LLVMInvalid {
  impl::CommonInitialSequence CIS = impl::TagT::LLVMInvalid;

public:
  constexpr explicit LLVMInvalid() {}
  constexpr bool operator==(const LLVMInvalid &O) const { return true; }
  static constexpr StringRef getAsmName() { return "DIOpLLVMInvalid"; }
...
class FromUIntIterator
  ...
  // Each iterator knows the End so we can transparently yield an LLVMInvalid if
  // we encounter an invalid expression. We don't keep a lot of these iterators
  // around, so the doubling in size shouldn't be significant. If it ever
  // becomes an issue, we could explore using the "sentinel" support from
  // RangeTS when/if that becomes available to make the actual end() an empty
  // struct.
