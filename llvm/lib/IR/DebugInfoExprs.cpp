...
struct OpInfo {
  using ConstructT = void (*)(DIOp::Op *, const uint64_t *);
  uint8_t EncodedSize = 1u;
  ConstructT Construct = [](DIOp::Op *Out, const uint64_t *) {
    new (Out) DIOp::LLVMInvalid();
  };
  constexpr OpInfo() = default;
  constexpr OpInfo(uint8_t EncodedSize, ConstructT Construct)
      : EncodedSize(EncodedSize), Construct(Construct) {}
};

static constexpr size_t NumOps = 0x1ff + 1;
static constexpr uint64_t OpKnownMask = 0x10ff;
static constexpr uint64_t OpUnknownMask = ~OpKnownMask;
static constexpr bool isUnknownOp(uint64_t Op) {
  return Op & OpUnknownMask;
}
static constexpr size_t indexOf(unsigned Op) {
  uint64_t HighBit = (Op & 0x1000) >> 4;
  return HighBit | (Op & 0xff);
}

static constexpr std::array<OpInfo, NumOps> makeOpInfos() {
  std::array<OpInfo, NumOps> OpInfos = {};

#define HANDLE_OP0(NAME, ENCODING)                                             \
  OpInfos[indexOf(ENCODING)] = OpInfo(                                         \
      1u, [](DIOp::Op *Out, const uint64_t *) { new (Out) DIOp::NAME(); });
#define HANDLE_OP1(NAME, ENCODING, ...)                                        \
  OpInfos[indexOf(ENCODING)] =                                                 \
      OpInfo(2u, [](DIOp::Op *Out, const uint64_t *In) {                       \
        new (Out) DIOp::NAME(In[1]);                                           \
      });
#define HANDLE_OP2(NAME, ENCODING, ...)                                        \
  OpInfos[indexOf(ENCODING)] =                                                 \
      OpInfo(3u, [](DIOp::Op *Out, const uint64_t *In) {                       \
        new (Out) DIOp::NAME(In[1], In[2]);                                    \
      });
#include "llvm/IR/DIOps.def"
...
static constexpr std::array<OpInfo, NumOps> OpInfos = makeOpInfos();


uint32_t DIOp::FromUIntIterator::getCurrentOpSize() const {
  assert(!isUnknownOp(*I));

  return OpInfos[indexOf(*I)].EncodedSize;
}

DIOp::Op DIOp::FromUIntIterator::operator*() const {
  const uint32_t RemainingSize = getRemainingSize();
  DIOp::Op Ret = LLVMInvalid();

  if (isUnknownOp(*I))
    return Ret;

  auto Info = OpInfos[indexOf(*I)];

  if (Info.EncodedSize > RemainingSize)
    return Ret;

  Info.Construct(&Ret, I);
  return Ret;
}

void DIOp::LLVMInvalid::toUIntVec(SmallVectorImpl<uint64_t> &Out) const {
  llvm::report_fatal_error("LLVMInvalid cannot be converted toUIntVec");
}
